...
const PAPER = [0xfa, 0xf7, 0xf2];   // --paper
const ACCENT = [0x6b, 0x4f, 0x2a];  // --accent
const RIB = [0xf0, 0xe6, 0xd6];     // --accent-soft, knocked out of the leaf
...
function svg() {
  ...
  <path d="M ${P(-HALF_LEN - STEM_LEN, 0)} L ${P(HALF_LEN, 0)}"
        stroke="#6b4f2a" stroke-width="${(STEM_W * 2 * 64).toFixed(2)}" stroke-linecap="round"/>
  <path d="M ${P(-HALF_LEN, 0)} L ${P(HALF_LEN * 0.86, 0)}"
        stroke="#f0e6d6" stroke-width="${(RIB_W * 2 * 64).toFixed(2)}" stroke-linecap="round"/>
</svg>
`;
}
